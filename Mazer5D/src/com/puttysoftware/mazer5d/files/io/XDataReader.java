...
public class XDataReader implements AutoCloseable {
	...
	if (line != null) {
	    final String[] split = XDataReader.splitLine(line);
	    XDataReader.validateOpeningTag(split[0], XDataConstants.OBJECT_ID_TAG);
	    XDataReader.validateClosingTag(split[2], XDataConstants.OBJECT_ID_TAG);
	...
	if (line != null && !line.equals("<" + this.docTag + ">")) { //$NON-NLS-1$ //$NON-NLS-2$
	    throw new UnexpectedTagException("Opening doc tag does not match: expected <" + this.docTag //$NON-NLS-1$
	...
	final String line = this.br.readLine();
	if (line != null && !line.equals("</" + this.docTag + ">")) { //$NON-NLS-1$ //$NON-NLS-2$
	    throw new UnexpectedTagException("Closing doc tag does not match: expected </" + this.docTag //$NON-NLS-1$
		    + ">, found " + line + "!"); //$NON-NLS-1$ //$NON-NLS-2$
	}
    }
