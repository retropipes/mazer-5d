...
class Bounds extends GenericWall {
    ...
    public void determineCurrentAppearance(final int x, final int y,
        ...
        final MazeObject mo1 = app.getMazeManager().getMazeObject(x - 1, y
                - 1, z, Layers.OBJECT);
        try {
            mo1Name = mo1.getName();
        ...
        final MazeObject mo3 = app.getMazeManager().getMazeObject(x - 1, y
                + 1, z, Layers.OBJECT);
        try {
            mo3Name = mo3.getName();
        } catch (final NullPointerException np) {
        ...
        final MazeObject mo7 = app.getMazeManager().getMazeObject(x + 1, y
                - 1, z, Layers.OBJECT);
        try {
            mo7Name = mo7.getName();
        } catch (final NullPointerException np) {
        ...
        final MazeObject mo9 = app.getMazeManager().getMazeObject(x + 1, y
                + 1, z, Layers.OBJECT);
        try {
            mo9Name = mo9.getName();
        } catch (final NullPointerException np) {
