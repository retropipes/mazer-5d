...
class MazeData {
	...
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
		    final MazeObject mo = this.getCell(y, x, z, Layers.OBJECT);
		    if (mo != null) {
	...
	for (x = 0; x < this.getColumns(); x++) {
	    for (y = 0; y < this.getRows(); y++) {
		final MazeObject mo = this.getCell(y, x, floor, Layers.OBJECT);
		if (mo != null) {
		    mo.tickTimer(y, x);
		}
	    }
	...
	final MazeObject[][][] tempStorage = new MazeObject[2 * r + 1][2 * r + 1][Layers.COUNT];
	try {
	    for (u = x - r; u <= x + r; u++) {
		for (v = y - r; v <= y + r; v++) {
	...
	final MazeStorage tempStorage = new MazeStorage(y, x, z, Layers.COUNT);
	...
	for (u = x - r; u <= x + r; u++) {
	    for (v = y - r; v <= y + r; v++) {
		try {
		    final boolean reactsToIce = this.getCell(u, v, z, Layers.OBJECT)
			    .isOfType(TypeConstants.TYPE_REACTS_TO_ICE);
		    if (reactsToIce) {
			final MazeObject there = this.getCell(u, v, z, Layers.OBJECT);
			if (there.getUniqueID().equals(MazeObjects.BARRIER_GENERATOR)) {
			    // Freeze the generator
			    this.setCell(GameObjects.createObject(MazeObjects.ICED_BARRIER_GENERATOR), u, v, z,
				    Layers.OBJECT);
			} else {
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
		    // Freeze the ground
		    this.setCell(GameObjects.createObject(MazeObjects.ICE), u, v, z, Layers.GROUND);
		} catch (final ArrayIndexOutOfBoundsException aioob) {
		    // Do nothing
		}
	    }
	...
	for (u = x - r; u <= x + r; u++) {
	    for (v = y - r; v <= y + r; v++) {
		try {
		    final boolean reactsToFire = this.getCell(u, v, z, Layers.OBJECT)
			    .isOfType(TypeConstants.TYPE_REACTS_TO_FIRE);
		    if (reactsToFire) {
			final MazeObject there = this.getCell(u, v, z, Layers.OBJECT);
			if (there.getUniqueID().equals(MazeObjects.BARRIER_GENERATOR)) {
			    // Enrage the generator
			    this.setCell(GameObjects.createObject(MazeObjects.ENRAGED_BARRIER_GENERATOR), u, v, z,
				    Layers.OBJECT);
			} else {
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
		    // Burn the ground
		    this.setCell(GameObjects.createObject(MazeObjects.DIRT), u, v, z, Layers.GROUND);
		} catch (final ArrayIndexOutOfBoundsException aioob) {
		    // Do nothing
		}
	    }
	...
	for (u = x - r; u <= x + r; u++) {
	    for (v = y - r; v <= y + r; v++) {
		try {
		    final boolean reactsToPoison = this.getCell(u, v, z, Layers.OBJECT)
			    .isOfType(TypeConstants.TYPE_REACTS_TO_POISON);
		    if (reactsToPoison) {
			final MazeObject there = this.getCell(u, v, z, Layers.OBJECT);
			if (there.getUniqueID().equals(MazeObjects.BARRIER_GENERATOR)) {
			    // Weaken the generator
			    this.setCell(GameObjects.createObject(MazeObjects.POISONED_BARRIER_GENERATOR), u, v, z,
				    Layers.OBJECT);
			} else {
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
		    // Poison the ground
		    this.setCell(GameObjects.createObject(MazeObjects.ICED_BARRIER_GENERATOR), u, v, z, Layers.GROUND);
		} catch (final ArrayIndexOutOfBoundsException aioob) {
		    // Do nothing
		}
	    }
	...
	for (u = x - r; u <= x + r; u++) {
	    for (v = y - r; v <= y + r; v++) {
		try {
		    final boolean reactsToShock = this.getCell(u, v, z, Layers.OBJECT)
			    .isOfType(TypeConstants.TYPE_REACTS_TO_SHOCK);
		    if (reactsToShock) {
			final MazeObject there = this.getCell(u, v, z, Layers.OBJECT);
			if (there.getUniqueID().equals(MazeObjects.BARRIER_GENERATOR)) {
			    // Shock the generator
			    this.setCell(GameObjects.createObject(MazeObjects.SHOCKED_BARRIER_GENERATOR), u, v, z,
				    Layers.OBJECT);
			} else {
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
		    // Shock the ground
		    this.setCell(GameObjects.createObject(MazeObjects.FORCE_FIELD), u, v, z, Layers.GROUND);
		} catch (final ArrayIndexOutOfBoundsException aioob) {
		    // Do nothing
		}
	    }
	...
	int u, v, l, uFix, vFix;
	final MazeObject[][][] preShuffle = new MazeObject[2 * r + 1][2 * r + 1][Layers.COUNT];
	...
	// Do the shuffle
	final MazeObject[][][] postShuffle = MazeData.shuffleObjects(preShuffle, r);
	...
	for (u = x - r; u <= x + r; u++) {
		...
		try {
			...
			if (chance == 1) {
			    // Grow a crevasse
			    this.setCell(GameObjects.createObject(MazeObjects.CREVASSE), u, v, z, Layers.OBJECT);
			}
		    }
		    final boolean isBreakable = this.getCell(u, v, z, Layers.OBJECT)
			    .isOfType(TypeConstants.TYPE_BREAKABLE_WALL);
		    if (isBreakable) {
			// Destroy the wall
			this.setCell(GameObjects.getEmptySpace(), u, v, z, Layers.OBJECT);
		    }
		    final boolean isWall = this.getCell(u, v, z, Layers.OBJECT).isOfType(TypeConstants.TYPE_PLAIN_WALL);
		    if (isWall) {
			// Crack the wall
			this.setCell(GameObjects.createObject(MazeObjects.CRACKED_WALL), u, v, z, Layers.OBJECT);
		    }
		    final boolean isCharacter = this.getCell(u, v, z, Layers.OBJECT)
	...
	final GenericLightModifier lg = (GenericLightModifier) GameObjects.createObject(MazeObjects.LIGHT_GEM);
	final GenericLightModifier dg = (GenericLightModifier) GameObjects.createObject(MazeObjects.DARK_GEM);
	boolean result = true;
	final int zLoc = Mazer5D.getBagOStuff().getGameManager().getPlayerManager().getPlayerLocationZ();
	...
	if (xDist <= this.visionRadius && yDist <= this.visionRadius) {
	    if (this.radialScan(x2, y2, zLoc, Layers.OBJECT, dg.getEffectRadius(), dg.getName())) {
		result = false;
	    } else {
		result = true;
	    }
	} else {
	    if (this.radialScan(x2, y2, zLoc, Layers.OBJECT, lg.getEffectRadius(), lg.getName())) {
		result = true;
	    } else {
		result = false;
	    }
	...
	final MazeObject obj = this.getCell(this.getFirstMovingFinishX(), this.getFirstMovingFinishY(),
		this.getFirstMovingFinishZ(), Layers.OBJECT);
	if (obj.getUniqueID().equals(MazeObjects.MOVING_FINISH)) {
	    final GenericTeleport gt = (GenericTeleport) obj;
	    gt.activate();
	}
	...
	// Pre-Pass
	final MazeObject pass1FillBottom = GameObjects.createObject(Prefs.getEditorDefaultFill());
	...
	for (e = 0; e < Layers.COUNT; e++) {
	    final MazeObject[] objectsWithoutPrerequisites = GameObjects.getAllWithoutPrerequisiteAndNotRequired(e);
		...
		for (x = 0; x < columns; x++) {
			...
			if (okay) {
			    this.setCell(GameObjects.createObject(placeObj.getUniqueID()), y, x, z, e);
	...
	for (int layer = 0; layer < Layers.COUNT; layer++) {
	    final MazeObject[] requiredObjects = GameObjects.getAllRequired(layer);
		...
		for (x = 0; x < requiredObjects.length; x++) {
			...
			if (currObj.shouldGenerateObject(maze, randomRow, randomColumn, z, w, layer)) {
			    this.setCell(GameObjects.createObject(currObj.getUniqueID()), randomColumn, randomRow, z,
				    layer);
	...
	final MazeObject pass1FillBottom = GameObjects.createObject(Prefs.getEditorDefaultFill());
	final MazeObject pass1FillTop = GameObjects.getEmptySpace();
	final MazeObject[] withoutRuleSets = GameObjects.getAllObjectsWithoutRuleSets();
	final MazeObject[] withRuleSets = GameObjects.getAllObjectsWithRuleSets();
	RandomRange r = null;
	int x, y, e;
	// Pass 1
	this.fillFloor(pass1FillBottom, pass1FillTop, z);
	...
	if (withoutRuleSets != null) {
	    ...
	    // Pass 3
	    for (int layer = 0; layer < Layers.COUNT; layer++) {
		final MazeObject[] requiredObjects = GameObjects.getAllRequiredSubset(withoutRuleSets, layer);
		if (requiredObjects != null) {
		    final RandomRange row = new RandomRange(0, this.getRows() - 1);
		    final RandomRange column = new RandomRange(0, this.getColumns() - 1);
		    int randomColumn, randomRow;
			...
			for (y = 0; y < generateHowMany; y++) {
			    ...
			    randomColumn = column.generate();
			    if (currObj.shouldGenerateObject(maze, randomRow, randomColumn, z, w, layer)) {
				this.setCell(GameObjects.createObject(currObj.getUniqueID()), randomColumn, randomRow,
					z, layer);
				currObj.editorGenerateHook(y, x, z);
			    } else {
				while (!currObj.shouldGenerateObject(maze, randomColumn, randomRow, z, w, layer)) {
	...
	if (withRuleSets != null) {
	    ...
	    // Pass N + 3
	    for (int layer = 0; layer < Layers.COUNT; layer++) {
		final MazeObject[] requiredObjects = GameObjects.getAllRequiredSubset(withRuleSets, layer);
		if (requiredObjects != null) {
		    final RandomRange row = new RandomRange(0, this.getRows() - 1);
		    final RandomRange column = new RandomRange(0, this.getColumns() - 1);
		    int randomColumn, randomRow;
	...
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
		    for (e = 0; e < Layers.COUNT; e++) {
			this.savedState.setDataCell(this.getCell(y, x, z, e), x, y, z, e);
		    }
		}
	    }
	}
	...
	for (x = 0; x < this.getColumns(); x++) {
		...
		for (z = 0; z < this.getFloors(); z++) {
		    for (e = 0; e < Layers.COUNT; e++) {
			this.setCell(this.savedState.getDataCell(x, y, z, e), y, x, z, e);
		    }
		}
	    }
	}
	...
	final int zLoc = Mazer5D.getBagOStuff().getGameManager().getPlayerManager().getPlayerLocationZ();
	try {
	    final MazeObject there = this.getCell(xLoc + dirMove[0], yLoc + dirMove[1], zLoc, Layers.OBJECT);
	    final MazeObject ground = this.getCell(xLoc + dirMove[0], yLoc + dirMove[1], zLoc, Layers.GROUND);
	    if (!there.isSolid() && !there.getName().equals("Player")) {
		this.setCell(block.getSavedObject(), xLoc, yLoc, zLoc, Layers.OBJECT);
		// Move the block
		block.setSavedObject(there);
		this.setCell(block, xLoc + dirMove[0], yLoc + dirMove[1], zLoc, Layers.OBJECT);
	...
	randomRow = row.generate();
	randomColumn = column.generate();
	final MazeObject currObj = this.getCell(randomRow, randomColumn, z, Layers.OBJECT);
	if (!currObj.isSolid()) {
	...
	this.setCell(GameObjects.createObject(MazeObjects.HOT_ROCK), x, y, z, Layers.GROUND);
    }
	...
	final MazeObject[][][] postShuffle = new MazeObject[2 * r + 1][2 * r + 1][Layers.COUNT];
    ...
    public void writeXMLLayeredTower(final MazeDataWriter writer) throws IOException {
	int y, x, z, e;
    ...
    public static MazeData readXMLMazeDataModelV1(final MazeDataReader reader, final int ver) throws IOException {
    ...
    public static MazeData readXMLMazeDataModelV2(final MazeDataReader reader, final int ver) throws IOException {
	int y, x, z, e, mazeSizeX, mazeSizeY, mazeSizeZ;
	mazeSizeX = reader.readInt();
	mazeSizeY = reader.readInt();
	mazeSizeZ = reader.readInt();
    ...
    public static MazeData readXMLMazeDataModelV3(final MazeDataReader reader, final int ver) throws IOException {
	int y, x, z, e, mazeSizeX, mazeSizeY, mazeSizeZ;
	mazeSizeX = reader.readInt();
	mazeSizeY = reader.readInt();
	mazeSizeZ = reader.readInt();
    ...
    public static MazeData readXMLMazeDataModelV4(final MazeDataReader reader, final int ver) throws IOException {
	int y, x, z, e, mazeSizeX, mazeSizeY, mazeSizeZ;
	mazeSizeX = reader.readInt();
	mazeSizeY = reader.readInt();
	mazeSizeZ = reader.readInt();
    ...
    public static MazeData readXMLMazeDataModelV5(final MazeDataReader reader, final int ver) throws IOException {
	int y, x, z, e, mazeSizeX, mazeSizeY, mazeSizeZ;
	mazeSizeX = reader.readInt();
	mazeSizeY = reader.readInt();
	mazeSizeZ = reader.readInt();
    ...
    public void readSavedTowerStateXML(final MazeDataReader reader, final int formatVersion) throws IOException {
	this.savedState = SavedState.readSavedStateXML(reader, formatVersion);
    }
}
