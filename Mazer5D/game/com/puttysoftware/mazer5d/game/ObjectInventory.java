...
public final class ObjectInventory implements Cloneable {
	...
	private static final GenericBow DEFAULT_BOW = (GenericBow) GameObjects.createObject(MazeObjects.BOW);
	...
	public boolean isItemCategoryThere(final int cat) {
		final MazeObject[] objects = GameObjects.getAllInventoryableObjectsMinusSpecial();
	...
	void fireStepActions() {
		if (!this.boots.getName().equals(ObjectInventory.DEFAULT_BOOTS.getName())) {
			this.boots.stepAction();
		}
		if (!this.amulet.getName().equals(ObjectInventory.DEFAULT_AMULET.getName())) {
	...
	public String[] generateInventoryStringArray() {
		final String[] result = new String[this.contents.length + this.bows.length + 2];
		StringBuilder sb;
	...
	public static ObjectInventory readInventory(final MazeDataReader reader, final MazeVersion formatVersion)
			throws IOException {
		...
		i.amulet = (GenericAmulet) GameObjects.readObject(reader, formatVersion);
		if (i.amulet == null) {
